...
VlnPlot(BMs.integrated,features = HLA)
...
NCP1 = MyUMAPPlot(BMs.integrated, label="NCP1")
NCP2 = MyUMAPPlot(BMs.integrated, label="NCP2")
NCP3 = MyUMAPPlot(BMs.integrated, label="NCP3")
NCP4 = MyUMAPPlot(BMs.integrated, label="NCP4")
cMOPs = MyUMAPPlot(BMs.integrated, label="cMOP")

p1 <- multiplot(NCP1,NCP2,NCP3,NCP4,cMOPs,cols = 2)

cairo_ps(file="/home/pg/Dropbox/Precursors/CoralDrawFigures/FigS7B_A_UMAP_all_genes.eps", width = 8, height = 10)
multiplot(NCP1,NCP2,NCP3,NCP4,cMOPs,cols = 2)
dev.off()
...
BMs.integrated = subset(BMs.integrated,idents=c('6','7'), invert = TRUE)
BMs.integrated = NormalizeData(BMs.integrated)
DimPlot(BMs.integrated,reduction="umap_learn50PC",label = T)
...
my_levels <- c("0","3","1","4","2","5");length(my_levels) ### 0.4
BMs.integrated@active.ident <- factor(x=BMs.integrated@active.ident,levels = my_levels)
...
genes <- c("CEBPE","IRF8","GFI1","CSF1R","FLT3","ELANE","MPO","CTSG")
cairo_ps(file="/home/simple/Dropbox/Precursors/CoralDrawFigures/NewCoreldraw/ViolinPlot_Neu_cMOP_Fig6.eps", width = 8, height = 5)
VlnPlot(BMs.integrated,features = genes,pt.size = 0) + scale_color_manual(values=cluster_col)
dev.off()

plot_list <- list()
feats <- c("CEBPE","IRF8","GFI1","CSF1R","FLT3","ELANE","PRTN3","MPO","CTSG")
for(i in 1:length(feats)){
  plot_list[[i]] <- VlnPlot(BMs.integrated, features=c(feats[[i]]),pt.size = 0) + geom_boxplot(width=0.1) +  theme(legend.position = 'none')
}
...
feats <- c("CEBPE","IRF8","GFI1","CSF1R","FLT3","ELANE","PRTN3","MPO","CTSG")
for(i in 1:length(feats)){
  plot_list[[i]] <- VlnPlot(BMs.integrated, features=c(feats[[i]]),pt.size = 0) + 
    stat_summary(fun = median, geom='point', size = 2, colour = "black", shape = 19) +  theme(legend.position = 'none')
}
CombinePlots(plot_list)
